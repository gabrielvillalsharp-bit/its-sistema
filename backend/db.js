...
function init() {
  ...
  // Director
  if (!db.prepare("SELECT id FROM usuarios WHERE rol='director' LIMIT 1").get()) {
    db.prepare(`INSERT INTO usuarios (id,nombre,apellido,ci,email,password_hash,rol) VALUES
      ('u_dir','Roberto','Martínez','1.111.111','director@its.edu.py',?,'director')`).run(bcrypt.hashSync('director123',10));
    db.prepare(`INSERT INTO usuarios (id,nombre,apellido,ci,email,password_hash,rol) VALUES
      ('u_adm','Secretaría','Académica','2.222.222','admin@its.edu.py',?,'admin')`).run(bcrypt.hashSync('admin123',10));
  }
